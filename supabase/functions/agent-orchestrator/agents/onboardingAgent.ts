...
export async function evaluateOnboardingAgent(
  ...
  try {
    ...
    const progress = await getUserOnboardingProgress(supabase, userId, userCreatedAt);
    ...
    const hasRecent = await hasRecentNotification(supabase, userId, 'onboarding', 3);
    if (hasRecent) {
      return {
        action: 'skip',
    ...
    const allClasses = await getAllClasses(supabase, userId);
    const activeClasses = await getActiveClasses(supabase, userId);
